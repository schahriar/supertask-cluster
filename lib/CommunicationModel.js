...
CommunicationModel.prototype.incoming = function STC_COM_INCOMING(id, message) {
    if(message.ticket && !message.type) {
        ...
        if(message.subtype === 'get') {
            ...
            if((message.partition.start !== undefined) && (message.partition.end !== undefined)) {
                this.autoBuffer(id, message.storeAs || message.name, BufferObject.get(message.partition.start, message.partition.end - message.partition.start), BufferObject.encoding, !BufferObject.immutable, null, callback);
            }else{
                this.autoBuffer(id, message.name, BufferObject.get(), BufferObject.encoding, !BufferObject.immutable, null, callback);
            }
        }else{
...
CommunicationModel.prototype.autoBuffer = function STC_AUTO_BUFFER(workerID, name, buffer, encoding, mutable, sendInChunks, callback) {
    var _this = this;
    var BufferProtoObject = new BufferTransfer(name, encoding, buffer.length, mutable);
    _this.bufferAllocate(workerID, BufferProtoObject, buffer.length, function(error, success, response) {
        if(error || !success) return callback(error || new Error("Failed to allocate buffer on Worker."));
        _this.buffer(workerID, BufferProtoObject, buffer, sendInChunks, callback);
    });
};

CommunicationModel.prototype.bufferAllocate = function STC_ALLOCATE(workerID, BufferProtoObject, size, callback) {
    this.send(workerID, BufferProtoObject.AllocatorObject(size), false, function(error, success, response) {
        if(error || !success) return callback(error || new Error("Failed to allocate buffer on Worker."));
        callback(error, success, response);
    });
    return BufferProtoObject;
};
...
CommunicationModel.prototype.buffer = function STC_BUFFER(workerID, BufferProtoObject, buffer, sendInChunks, callback) {
    ...
    _this.on('CLUSTER_CALLBACK::' + workerID + "::STC_BUFFER:" + BufferProtoObject.name, function _STC_BUFFER_VERIFY(response) {
        ...
        var BufferLoc = BufferMap.get(BufferProtoObject.name);
        if(!BufferLoc) BufferLoc = {};
        if(!BufferLoc[workerID]) {
            BufferLoc[workerID] = true;
            BufferMap.set(BufferProtoObject.name, BufferLoc);
        }
        // TODO
        // Add buffer remove (set to null on Worker)
        // --------------------
        callback(null, { stored_as: BufferProtoObject.name });
    });
};
